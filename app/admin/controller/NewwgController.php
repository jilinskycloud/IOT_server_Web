...
class NewwgController extends AdminBaseController
{
    //控制网关是否保存更多sn_id的开关
...
//            }
            ...
            if ($result !== false) {
                    ...
                    $arrFiled['type'] = 4;
                    $arrFiled['is_send'] = 0;
                    $arrFiled['data'] = json_encode($data);
    ...
    public function fjdel(){
        ...
        if (Db::name('fcinfo')->delete($id) !== false) {
            $this->success("删除成功！");
        } else {
            $this->error("删除失败！");
        }
    ...
    public function getalltempdata(){
        ...
        if ($this->request->isGet()) {
            ...
            $list = Db::name('dt_loc')->order('id desc')->where($where)->paginate(20, false, ['query' => request()->param()]);

        $data=$list->items();
//        foreach($data as $key=>$val){
//            $data[$key]['create_time'] = date("Y-m-d H:i:s", $val['strotime']);
//        }
        $this->assign('page', $list->render());
        $this->assign('data', $data);
        $this->assign('data_gw_id', $gw_id);
    ...
    public function getgatelist(){
        ...
        if ($this->request->isPost()) {
            ...
            //有查询条件
            if (isset($data['fcname']) && $data['fcname']) {
                $where="wg  like '%".$data['fcname']."%'";
                $where.=" or ip like '%".$data['fcname']."%'";
            }

        }
        $list = Db::name('dt_loc')->group('wg')->where($where)->paginate(20, false, ['query' => request()->param()]);
        $data=$list->items();
        $time = strtotime('-120 seconds');
        foreach($data as $key=>$val){
            $l=Db::name('dt_loc')->order('id desc')->where(array('wg'=>$val['wg']))->find();
            $data[$key]['id']=$l['id'];
            $data[$key]['ip']=$l['ip'];
            $data[$key]['wg']=$l['wg'];
            $data[$key]['xh']=$l['xh'];
            $data[$key]['dc']=$l['dc'];
            $data[$key]['time']=$l['time'];
            $data[$key]['create_time']=$l['create_time'];
            if ($l['strotime'] < $time) {
                $data[$key]['status_name']='失效';
                $data[$key]['status']=2;
            } else {
